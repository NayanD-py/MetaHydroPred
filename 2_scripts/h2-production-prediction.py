...
def calculate_regression_scores(Y_test, Y_pred, metrics=None):

    available_metrics = {
        "mean_absolute_error": lambda: mean_absolute_error(Y_test, Y_pred),
        "mean_absolute_percentage_error": lambda: mean_absolute_percentage_error(Y_test, Y_pred),
        "mean_squared_error": lambda: mean_squared_error(Y_test, Y_pred),
        "root_mean_squared_error": lambda: np.sqrt(mean_squared_error(Y_test, Y_pred)),
        "r2_score": lambda: r2_score(Y_test, Y_pred),
        "pearson_correlation": lambda: pearsonr(Y_test, Y_pred)[0],
        "explained_variance_score": lambda: explained_variance_score(Y_test, Y_pred),
        "max_error": lambda: max_error(Y_test, Y_pred),
        "median_absolute_error": lambda: median_absolute_error(Y_test, Y_pred),
        "mean_squared_log_error": lambda: mean_squared_log_error(Y_test, Y_pred)
        }
    
    if metrics is None:
        metrics = available_metrics.keys()

    scores = {metric: available_metrics[metric]() for metric in metrics if metric in available_metrics}
...
def predict_h2_production_rate(
    train_dataset_path,
    test_dataset_path,
    meta_train_path,
    target_column,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Predict h2 production rate using MetaHydroPred.")
    ...
    target_column = 'H2 production rate'  # Default target column
    output_csv_path = args.output_csv_path

    print(f"Predicting H2 production rate for substrate type: {substrate_type}")   
...
#--------------------------------------------------------------------------------
    ...
    if substrate_type == "all-organic":
        ...
        # All-organic 
        train_dataset_path = "../1_data/h2-production-rate/benchmark-dataset/h2_production_rate_train.csv"
        test_dataset_path = "../1_data/h2-production-rate/benchmark-dataset/h2_production_rate_test.csv"
        meta_train_path = "../1_data/h2-production-rate/meta-feature-train/all-organic/MF-6-train.csv"



        baseline_models_dir_BF1 = "../3_saved-models/h2-production-rate-final/all-organic/baseline-models/BF-1/*.joblib"
        baseline_models_dir_BF3 = "../3_saved-models/h2-production-rate-final/all-organic/baseline-models/BF-3/*.joblib"

        meta_model_path = "../3_saved-models/h2-production-rate-final/all-organic/meta-model/MetaHydroPred-h2-production-rate-all-organic.joblib"

        output_csv_path = "h2_production_rate_predictions_all-organic.csv"

        prediction=predict_h2_production_rate_all_organic(
                train_dataset_path=train_dataset_path,
                test_dataset_path=test_dataset_path,
                meta_train_path= meta_train_path,
    ...
    #--------------------------------------------------------------------------------
        ...
        # Acetate
        train_dataset_path = "../1_data/h2-production-rate/benchmark-dataset/h2_acetate_train.csv"
        test_dataset_path = "../1_data/h2-production-rate/benchmark-dataset/h2_acetate_test.csv"
        meta_train_path = "../1_data/h2-production-rate/meta-feature-train/acetate/MF-1-train.csv"



        baseline_models_dir_BF1 = "../3_saved-models/h2-production-rate-final/acetate/baseline-models/BF-1/*.joblib"


        meta_model_path = "../3_saved-models/h2-production-rate-final/acetate/meta-model/MetaHydroPred-h2-production-rate-acetate.joblib"

        output_csv_path = 'h2_production_rate_predictions-acetate.csv'

        predictions = predict_h2_production_rate_acetate(
        train_dataset_path=train_dataset_path,
        test_dataset_path=test_dataset_path,
        meta_train_path=meta_train_path,
        target_column="H2 production rate",
    ...
    #--------------------------------------------------------------------------------
        ...
        #  Complex-substrate
        train_dataset_path = "../1_data/h2-production-rate/benchmark-dataset/h2_complex_substance_train.csv"
        test_dataset_path = "../1_data/h2-production-rate/benchmark-dataset/h2_complex_substance_test.csv"
        meta_train_path = "../1_data/h2-production-rate/meta-feature-train/complex-substrate/MF-1-train.csv"



        baseline_models_dir_BF1 = "../3_saved-models/h2-production-rate-final/complex-substrate/baseline-models/BF-1/*.joblib"

        meta_model_path = "../3_saved-models/h2-production-rate-final/complex-substrate/meta-model/MetaHydroPred-h2-production-rate-complex-substrate.joblib"

        output_csv_path = 'h2_production_rate_predictions-complex-substrate.csv'

        predictions = predict_h2_production_rate_complex_substrate(
        train_dataset_path=train_dataset_path,
        test_dataset_path=test_dataset_path,
        meta_train_path=meta_train_path,
        target_column="H2 production rate",
